...
var articleDescription1 = "Desc for art 1";
var articlePrice1 = 10;
var articleName2 = "article 2";
var articleDescription2 = "Desc for art 2";
...
it("should let us sell a first article", function(){
  return ChainList.deployed().then(function(instance){
    ...
    assert.equal(receipt.logs[0].args.indexed_id.toNumber(),1,"id must be 1");
...
it("should let us sell a second article", function(){
 return ChainList.deployed().then(function(instance){
   ...
   assert.equal(receipt.logs.length,1,"one event should have been triggered");
   assert.equal(receipt.logs[0].event,"LogSellArticle","event should be LogSellArticle");
   assert.equal(receipt.logs[0].args.indexed_id.toNumber(),2,"id must be 2");
   assert.equal(receipt.logs[0].args._seller,seller,"event seller must be " + seller);
  ...
  it("should buy an article", function(){
    return ChainList.deployed().then(function(instance){
      chainListInstance = instance;
      //record balances of seller and buyers before the bUy
      ...
      assert.equal(receipt.logs[0].args.indexed_id.toNumber(),1,"article id must be 1");
      assert.equal(receipt.logs[0].args._seller,seller,"event seller must be " + seller);
      ...
      //check the effect of buy on balance of buyer and seller, accounting for Gaz
      assert(sellerBalanceAfterBuy = sellerBalanceBeforeBuy + articlePrice1,"seller shoud have earned " + articlePrice1 + " eth");
      assert(buyerBalanceAfterBuy <= buyerBalanceBeforeBuy - articlePrice1,"buyer shoud have paid " + articlePrice1 + " eth");
